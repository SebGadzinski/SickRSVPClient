		...
		<q-list>
			<template v-if="user">
				...
				<q-item clickable v-ripple>
					...
					<q-item-section>
						<q-btn color="primary" @click="handleRefreshSession" class="full-width"
							:label="$t('Refresh')" />
					</q-item-section>
				</q-item>
				<!-- View Profile -->
				<q-item clickable v-ripple>
					...
					<q-item-section>
						<q-btn color="primary" to="/profile" class="full-width" :label="$t('Profile')"></q-btn>
					</q-item-section>
				</q-item>
				<!-- LOGOUT -->
				<q-item clickable v-ripple>
					...
					<q-item-section>
						<q-btn color="primary" @click="handleLogout" class="full-width" :label="$t('Logout')" />
					</q-item-section>
				</q-item>
				<!-- Confirmation -->
				<q-item v-if="!user.emailConfirmed" clickable v-ripple>
					<q-item-section>{{ $t("Confirmation") }}</q-item-section>
					<q-item-section>
						<q-btn color="primary" @click="handleConfirmation" class="full-width"
							:label="$t('Send Email')" />
					</q-item-section>
				</q-item>
				<!-- Reset Password -->
				<q-item clickable v-ripple>
					...
					<q-item-section>
						<q-btn color="primary" to="/auth/forgotPassword" class="full-width"
							:label="$t('Reset')"></q-btn>
					</q-item-section>
				</q-item>
			</template>
			...
			<q-item tag="label" v-ripple>
				...
				<q-item-section>
					<q-btn-dropdown color="primary" class="full-width" split :label="langText">
						<q-list>
							<q-item v-for="(lan, index) in languages" :key="index" clickable v-close-popup
			...
			<!-- CHECK FOR UPDATE -->
			<q-item clickable v-ripple @click="checkForUpdate" v-if="platform != 'web'">
				<q-item-section>
					{{ $t("Version") }}:
					<span class="text-weight-bold">{{ $t(this.version) }}</span>
				</q-item-section>
				<q-item-section>
					<q-btn color="primary" class="full-width" :label="$t('Check for Update')" />
				</q-item-section>
			</q-item>
			...
			<!-- NOTIFICATIONS -->
			<q-item tag="label" v-ripple v-if="platform != 'web' && this.user">
				<q-item-section>
				...
				<q-item-section side top>
					<q-toggle color="primary" v-model="_notificationsEnabled" val="bool" />
				</q-item-section>
			</q-item>
			...
			<div class="full-width bg-negative text-white rounded-borders non-selectable row flex-center"
				style="height: 30px" v-if="message">
				<div>{{ message }}</div>
			</div>
